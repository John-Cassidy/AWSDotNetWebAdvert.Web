...
namespace AWSDotNetWebAdvert.Web.ServiceClients
{
    public class AdvertApiClient : IAdvertApiClient {
        ...
        public async Task<AdvertResponse> Create(CreateAdvertModel model) {
            ...
            var createAdvertResponseAsString = await response.Content.ReadAsStringAsync().ConfigureAwait(false);
            var createAdvertResponse = JsonSerializer.Deserialize<CreateAdvertResponse>(createAdvertResponseAsString, new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
